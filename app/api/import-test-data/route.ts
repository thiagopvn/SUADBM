...
export async function POST() {
  try {
    // Import all test data directly to Firebase
    ...
    return NextResponse.json({
      ...
      data: {
        creditos: Object.keys(testData.creditos).length,
        despesas: Object.keys(testData.despesas).length,
        prestacoes: Object.keys(testData.prestacoesContas).length,
        metasAcoes: Object.keys(testData.metasAcoes).length
      }
    });
  } catch (error) {
...
export async function GET() {
  return NextResponse.json({
    message: 'Use POST para importar dados de teste',
    endpoint: '/api/import-test-data',
    method: 'POST'
  });
}
